...
[image1]: models/fig_dataset_training.png "Distribution of images per breed"
[image2]: models/fig_dataset_sample_subset_categories.png "Sample of dataset images"
[image3]: models/fig_dataset_sample_subset_category_variation.png "Sample of dataset images"
[image4]: models/weights_best_Resnet50_fit_history.png "Model training history"
[image5]: models/weights_best_Resnet50_confusion_matrix.png "Confusion matrix"
...
4. Geneate the bottleneck features for the dog dataset using the script `generate_bottleneck.py`. Alternatively, one can load the pre-compute [DogResnet50 bottleneck features](https://s3-us-west-1.amazonaws.com/udacity-aind/dog-project/DogResnet50Data.npz) for the dog dataset. Place the data in the sub-directory `data/`. If downloading the DogResnet50 bottleneck features, the script `trainClassifier.py` has to be modified to correctly load the bottleneck features..
   ```
...
5. Set up a virtual environment and install the python packages requirements.
    ```
    python3 -m venv .venv
...
6. Switch Keras backend to TensorFlow
    ```
